...
from xmljson import badgerfish as bf  # pylint: disable=E0401
from xml.etree.ElementTree import fromstring

ZILLOW_ENDPOINT = 'http://www.zillow.com/webservice'
ZWS_ID = 'X1-ZWz18667b3o363_5taq0'
...
def getSearchResults(address, citystatezip, rentzestimate=False):
    ...
    response = requests.get(
        build_url(GET_SEARCH_RESULTS_API_NAME), params=payload)
    res_json = loads(dumps(bf.data(fromstring(response.text))))

    # Extract info from response.
    for key in res_json:
        if (res_json[key] is not None
            and res_json[key]['response'] is not None
                and res_json[key]['response']['results'] is not None):
...
def getUpdatedPropertyDetails(zpid):
    payload = {'zws-id': ZWS_ID,
               'zpid': zpid}
    response = requests.get(
        build_url(GET_UPDATED_PROPERTY_DETAILS_API_NAME), params=payload)
    res_json = loads(dumps(bf.data(fromstring(response.text))))
